...
class BilibiliDownloader:
    def __init__(self, user_id):
        ...
        # Load Bilibili cookies from environment variables and create cookie file
        self.cookie_file = self._create_cookie_file()
    
    def _create_cookie_file(self):
        """
        Create a Netscape format cookie file from environment variables.
        
        Returns:
            str: Path to cookie file, or None if cookies not available
        """
        sessdata = os.environ.get("BILIBILI_SESSDATA", "")
        bili_jct = os.environ.get("BILIBILI_BILI_JCT", "")
        buvid3 = os.environ.get("BILIBILI_BUVID3", "")
        
        if not (sessdata and bili_jct and buvid3):
        ...
        cookie_file_path = os.path.join(self.download_dir, ".bilibili_cookies.txt")
        ...
        try:
            with open(cookie_file_path, 'w') as f:
                ...
                # Using a far future expiration date (year 2030)
                expiration = "1893456000"  # 2030-01-01
                
                # SESSDATA cookie
                f.write(f".bilibili.com\tTRUE\t/\tTRUE\t{expiration}\tSESSDATA\t{sessdata}\n")
                # bili_jct cookie  
                f.write(f".bilibili.com\tTRUE\t/\tTRUE\t{expiration}\tbili_jct\t{bili_jct}\n")
                # buvid3 cookie
                f.write(f".bilibili.com\tTRUE\t/\tFALSE\t{expiration}\tbuvid3\t{buvid3}\n")
            
            logger.info(f"Bilibili cookie file created successfully: {cookie_file_path}")
    ...
    def get_latest_videos(self, limit=10):
        ...
        try:
            ...
            # Add realistic browser headers and cookies to avoid anti-bot detection
            ydl_opts = {
                'quiet': True,
                'no_warnings': True,
